/* ════════════════════════════════════════════════════
   ...
   (same file where /api/book-consulting lives)
════════════════════════════════════════════════════ */
...
<div style="font-family:DM Sans,sans-serif;max-width:580px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e5e7eb;">
  <div style="background:linear-gradient(135deg,#1a1814,#2a2318);padding:32px 36px;">
    <div style="font-family:Syne,sans-serif;font-size:20px;font-weight:900;color:#fff;">ImpactGrid</div>
    <div style="color:rgba(255,255,255,.55);font-size:13px;margin-top:4px;">New Contact Message</div>
  </div>
  <div style="padding:32px 36px;">
    <h2 style="font-size:18px;font-weight:800;color:#111;margin:0 0 20px;font-family:Syne,sans-serif;">✉️ New Message</h2>
    <table style="width:100%;border-collapse:collapse;font-size:14px;">
      <tr>
        <td style="padding:9px 0;color:#888;width:160px;vertical-align:top;border-bottom:1px solid #f0f0f0;">Name</td>
        <td style="padding:9px 0;color:#111;font-weight:600;border-bottom:1px solid #f0f0f0;">${firstName}${lastName ? ' ' + lastName : ''}</td>
      </tr>
      <tr>
      ...
      <tr>
        <td style="padding:9px 0;color:#888;vertical-align:top;border-bottom:1px solid #f0f0f0;">Topic</td>
        <td style="padding:9px 0;color:#111;font-weight:600;border-bottom:1px solid #f0f0f0;">${subjectLabel}</td>
      </tr>
      <tr>
  ...
  <div style="background:#fafafa;padding:20px 36px;font-size:12px;color:#aaa;border-top:1px solid #e5e7eb;">
    Sent via ImpactGrid Contact form · impactgridgroup.com
  </div>
</div>`;
  ...
  try {
    const resendRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'ImpactGrid <support@impactgrid.com>',
        to: ['Ogunmoyerodara@gmail.com'],
        reply_to: email,
        subject: `✉️ New Message: ${firstName}${lastName ? ' ' + lastName : ''} — ${subjectLabel}`,
        html: emailHtml
      })
    });
