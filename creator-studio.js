...
async function fetchTrends() {
  ...
  function mapTrend(t, i) {
    ...
    var plat = src === 'youtube' ? 'yt'
      : src === 'tiktok'   ? 'tt'
      : src === 'twitter'  ? 'xtwitter'   // X / Twitter — separate from Google
      : src === 'cross'    ? 'cross'
      : 'gt';
    var platLbl = src === 'youtube' ? 'YouTube'
      : src === 'tiktok'  ? 'TikTok'
      : src === 'twitter' ? 'X / Twitter'
      : src === 'cross'   ? 'Cross-Platform'
      : 'Google';
    return {
      topic:        t.topic,
      ...
      xPrediction:  t.twitter_prediction   || 0,
      igReason:     t.instagram_reason     || '',
      xReason:      t.twitter_reason       || '',
...
function trendItemHTML(t) {
  ...
  var platCls = t.plat === 'yt' ? 'plat-yt' : t.plat === 'tt' ? 'plat-tt' : t.plat === 'xtwitter' ? 'plat-x' : 'plat-gt';
...
function renderDashTrends() {
  ...
  var platColors = { tt: '#ff6464', yt: '#FFD700', gt: '#78b4ff', cross: '#4FB3A5', xtwitter: '#e7e9ea' };
  ...
  el.innerHTML = picks.map(function(t, idx) {
    ...
    var actionHint = t.plat === 'tt'       ? 'Post a 30–60s hook video today'
                   ...
                   : t.plat === 'xtwitter' ? 'Thread or text post performs well here'
