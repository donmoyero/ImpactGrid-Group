...
function onTopicInput(){
  clearTimeout(dTimer);
  var _ti=document.getElementById('topicInput');
  var _gb=document.getElementById('genBtn');
  var v=_ti ? _ti.value.trim() : '';
  if(_gb) _gb.disabled=v.length<4;
  if(v.length<4){resetIntel();return;}
  dTimer=setTimeout(function(){runDetect(v);},350);
}

function resetIntel(){
  var ic=document.getElementById('intelCard');
  var iv=document.getElementById('intelVal');
  var il=document.getElementById('intelLbl');
  var ap=document.getElementById('assetPreviewField');
  if(ic) ic.classList.remove('active');
  if(iv) iv.textContent='Waiting for topic…';
  if(il) il.textContent='DIJO INTELLIGENCE';
  for(var i=0;i<5;i++){var s=document.getElementById('iseg'+i);if(s)s.classList.remove('on');}
...
function runDetect(topic){
  ...
  var lbl=document.getElementById('intelLbl');
  var val=document.getElementById('intelVal');
  if(card) card.classList.add('active');
  if(lbl) lbl.textContent='THEME DETECTED';
  if(val) val.textContent=T.label+' · '+T.mood;
  ...
  for(var i=0;i<5;i++){var s=document.getElementById('iseg'+i);if(s)s.classList.toggle('on',i<segs);}
...
function showAssetPreview(theme){
  ...
  var field=document.getElementById('assetPreviewField');
  if(!T||!row||!field) return;
  row.innerHTML='';
...
async function generate(){
  var _topicEl=document.getElementById('topicInput');
  var topic=_topicEl ? _topicEl.value.trim() : (ST.topic||'').trim();
  if(!topic){toast('⚠️ Add a topic first');if(_topicEl)_topicEl.focus();return;}
  ...
  // ── Null-safe DOM refs (wizard merge may rename or defer these elements) ──
  var _platEl    = document.getElementById('platSelect');
  var _toneEl    = document.getElementById('toneSelect');
  var _btn       = document.getElementById('genBtn');
  var _emptyState= document.getElementById('emptyState');
  var _slideWrap = document.getElementById('slideWrap');
  var _loadingOv = document.getElementById('loadingOv');
  var _loadingHint=document.getElementById('loadingHint');

  var platform   = _platEl  ? _platEl.value  : (ST.platform || 'Instagram');
  var tone       = _toneEl  ? _toneEl.value  : (ST.tone     || 'Bold & Direct');
  var count      = ST.count || 7;

  if(_btn)        { _btn.innerHTML='<div class="spin"></div> Generating…'; _btn.disabled=true; }
  if(_emptyState) _emptyState.style.display='none';
  if(_slideWrap)  _slideWrap.style.display='block';
  if(_loadingOv)  _loadingOv.classList.add('show');

  var hints=['Detecting theme…','Scoring images with AI vision…','Writing punchy copy…','Designing layouts…','Adding captions + hashtags…'];
  var hi=0,hTimer=setInterval(function(){hi=(hi+1)%hints.length;if(_loadingHint)_loadingHint.textContent=hints[hi];},1800);
  try{
  ...
  if(_loadingOv) _loadingOv.classList.remove('show');
  ST.cur=0;
  ...
  if(_btn){
    _btn.innerHTML='<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg> Regenerate';
    _btn.disabled=false;
  }
...
function renderSlide(){
  ...
  if(sNumEl) sNumEl.textContent=(ST.cur+1)+' / '+ST.slides.length;
...
/* ─────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────── */
function buildStrip(){
  var strip=document.getElementById('strip')||document.getElementById('sDualThumb')||document.getElementById('thumbStrip');
  if(!strip) return;
...
function updateCounter(){var el=document.getElementById('slideCtr');if(el) el.textContent=ST.slides.length?(ST.cur+1)+' / '+ST.slides.length:'0 / 0';}
function chgCount(d){ST.count=Math.max(3,Math.min(12,ST.count+d));var el=document.getElementById('cntVal');if(el) el.textContent=ST.count;}
function prefill(t){var el=document.getElementById('topicInput');if(el){el.value=t;onTopicInput();}}
function zoom(d){ST.zoom=Math.max(40,Math.min(150,ST.zoom+d));var lbl=document.getElementById('zoomLbl');var cvs=document.getElementById('slideCanvas');if(lbl) lbl.textContent=ST.zoom+'%';if(cvs) cvs.style.transform='scale('+ST.zoom/100+')';}
function setFmt(f){ST.format=f;['square','portrait','landscape'].forEach(function(x){var el=document.getElementById('fmt'+x.charAt(0).toUpperCase()+x.slice(1));if(el) el.classList.toggle('on',x===f);});var c=document.getElementById('slideCanvas');if(c) c.className='slide-canvas'+(f!=='square'?' '+f:'');}
...
function fillEdit(){
  ...
  var editNumEl=document.getElementById('editNum');
  if(editNumEl) editNumEl.textContent='Slide '+(ST.cur+1);
...
function dzOver(e){e.preventDefault();var dz=document.getElementById('dzone');if(dz) dz.classList.add('over');}
function dzLeave(){var dz=document.getElementById('dzone');if(dz) dz.classList.remove('over');}
function dzDrop(e){e.preventDefault();var _dz=document.getElementById('dzone');if(_dz) _dz.classList.remove('over');var f=e.dataTransfer.files[0];if(!f||!f.type.startsWith('image/')) return;var r=new FileReader();r.onload=function(ev){ST.userImages[ST.cur]=ev.target.result;renderSlide();buildStrip();toast('🖼️ Image dropped on slide '+(ST.cur+1));};r.readAsDataURL(f);}
...
function openExport(){var m=document.getElementById('exportModal');if(m) m.classList.add('show');}
function closeExport(){var m=document.getElementById('exportModal');if(m) m.classList.remove('show');}
